...
Ed = 600;     % Tensão maxima do barramento CC
Cap = 4.4E-3;
...
dtriangle = Ed/htriangle; % Derivada da onda triangular (dV/dT)
...
while(t < tf)
    ...
    eg_1 = vg_ref*cos(w_ref*t);
    eg_3 = vg_ref*cos(w_ref*t - 2*pi/3);
    eg_2 = vg_ref*cos(w_ref*t + 2*pi/3);
    ...
    if t >= ttriangle   
        ...
        % Controlador PI das correntes Ig
        vc_erro = vc_ref/sqrt(3) - vc;
        P_erro = Kp * vc_erro;
        I_erro = I_erro + htriangle*Ki*vc_erro;
...
%         end
        ...
        % Correntes de referencia para obter as tensoes
        ig1_ref = Ig * sqrt(2) * cos(w_ref*t);
        ig2_ref = Ig * sqrt(2) * cos(w_ref*t + 2*pi/3);
        ig3_ref = Ig * sqrt(2) * cos(w_ref*t - 2*pi/3);
        
        % Diferença: corrente de referencia e corrente real
        ig1_erro = (ig1_ref - ig1);
        ig2_erro = (ig2_ref - ig2);
        ig3_erro = (ig3_ref - ig3);

        %Tensões de referencia na entrada
        Vg1_ref = eg_1 - Rg*ig1 - (Lg/htriangle)*ig1_erro;
        Vg3_ref = eg_3 - Rg*ig3 - (Lg/htriangle)*ig3_erro;
        Vg2_ref = eg_2 - Rg*ig2 - (Lg/htriangle)*ig2_erro;
...
%         Vl3_ref = vl_ref*cos(w_ref*t + thetal_ref - 2*pi/3);
    ...
    ic = ig1*qg1 - il1*ql1 + ig3*qg3 - il3*ql3;
    vc = vc + ic*(h/Cap);
   % while (t>ti && t<tf)
...
%---- Tensão no capacitor e barramento
figure('Name','Cap')
plot(Ts,vcs,'r-','LineWidth',2),zoom
...
% %---- Tensoes de referencia na entrada
% figure('Name','Vgx_{ref}')
% plot(Ts,ig1_refs,Ts,ig2_refs, Ts,ig3_refs,'r-','LineWidth',2),zoom
% title('Vgx_{ref}')
% legend('Vg1_{ref}','Vg2_{ref}','Vg3_{ref}')
% xlabel("Tempo (s)")
% ylabel("Tensão (V)")
% grid minor

%---- Tensão e corrente da rede ----
figure('Name','eg1 e ig1')
plot(Ts,eg1s,Ts,10*ig1s,'r-','LineWidth',2),zoom
title('Tensão e Corrente da rede')
legend('eg1','ig1')
xlabel("Tempo (s)")
ylabel("Entrada")
grid minor
